...
enum spp_capture_type {
	TYPE_UNUSE,  /* Not used */
	TYPE_RECEIVE,/* thread type receive */
...
/* Parse options for limit file size */
static int
		...
		struct spp_iterate_core_params *params)
{
	...
	char name[COMPRESS_FILEPATH_LENGTH + COMPRESS_FILEDATE_LENGTH];
	struct spp_port_index rx_ports[1];
	int rx_num = 0;

	RTE_LOG(DEBUG, PCAP, "status core[%d]\n", lcore_id);
	...
	role_type = CAPTURE_THREAD_TYPE_STRINGS[info->type];
			       ...
			       int src_len)
{
	...
	if (LZ4F_isError(compress_len)) {
		RTE_LOG(ERR, PCAP, "Compression failed: error %zd\n",
							compress_len);
		return SPP_RET_NG;
	}
				   ...
				   enum comp_file_generate_mode mode)
{
	...
	RTE_LOG(DEBUG, PCAP, "Buffer size is %zd bytes, header size %zd "
...
static int pcap_proc_receive(int lcore_id)
{
	...
	if (g_capture_request == SPP_CAPTURE_IDLE) {
		if (info->status == SPP_CAPTURE_RUNNING) {
			RTE_LOG(DEBUG, PCAP, "recive[%d], run->idle\n",
								lcore_id);
	...
	if (info->status == SPP_CAPTURE_IDLE) {
		...
		RTE_LOG(DEBUG, PCAP, "recive[%d], idle->run\n", lcore_id);
		RTE_LOG(DEBUG, PCAP, "recive[%d], start time=%s\n",
...
static int pcap_proc_write(int lcore_id)
{
	...
	if (g_capture_status == SPP_CAPTURE_IDLE) {
		if (info->status == SPP_CAPTURE_RUNNING) {
			RTE_LOG(DEBUG, PCAP, "write[%d] run->idle\n",
								lcore_id);
			info->status = SPP_CAPTURE_IDLE;
	...
	if (info->status == SPP_CAPTURE_IDLE) {
		RTE_LOG(DEBUG, PCAP, "write[%d] idle->run\n", lcore_id);
		info->status = SPP_CAPTURE_RUNNING;
	...
	while (1) {
		if (spp_get_core_status(lcore_id) == SPP_CORE_STOP_REQUEST) {
			if (pcap_info->status == SPP_CAPTURE_IDLE)
				break;
			if (pcap_info->type == TYPE_RECEIVE)
...
main(int argc, char *argv[])
{
	...
	/**
	 * TODO(Yutaka.Ogasawara)
	 * Shall remain in effect. After debugging patch release.
	 */
	/* set_user_log_debug(RTE_LOG_INFO); */
	...
	while (1) {
		...
		RTE_LOG(DEBUG, PCAP, "Recv port type=%d, no=%d, port_id=%d\n",
		...
		RTE_LOG(DEBUG, PCAP, "Ring port name=%s, flags=0x%x\n",
